...
internal sealed class ResourceRegistry(AssemblyAnalyzer analysis)
{
    private const string AssembliesUri = "inspector://assemblies";
    ...
    public object[] ListResources()
    {
        var list = new List<object>
        {
            new
            {
                ...
                description = "Assemblies loaded in the DotNet Inspector MCP cache.",
                mimeType = "text/plain"
            }
        };
        ...
        foreach (var path in analysis.GetCachedAssemblyPaths())
        {
            ...
            list.Add(new
            {
                uri = $"inspector://assembly?path={encoded}&view=summary",
            ...
            list.Add(new
            {
                uri = $"inspector://assembly?path={encoded}&view=types",
    ...
    public async Task<string> ReadResourceAsync(string uriText)
    {
        ...
        if (!Uri.TryCreate(uriText, UriKind.Absolute, out var uri) || !string.Equals(uri.Scheme, "inspector", StringComparison.OrdinalIgnoreCase))
        ...
        if (!string.Equals(uri.Host, "assembly", StringComparison.OrdinalIgnoreCase))
            throw new InvalidOperationException($"Unsupported inspector resource host: {uri.Host}");
        ...
        {
            "summary" => await analysis.GetAssemblySummaryAsync(path),
            "types" => await analysis.ListTypesAsync(path, namespaceFilter: null, includeNested: true),
