...
internal static partial class AssemblyTools
{
    ...
    [McpTool("get_class_pointer", "Parse Il2CppDumper's script.json to get the static TypeInfo/Il2CppClass* pointer offset for external memory scanning.")]
    public static async Task<ToolCallResult> GetClassPointer(
        ToolContext ctx,
        [ToolParam("Path to the script.json file generated by Il2CppDumper.", Required = true)]
        string scriptJsonPath,
        [ToolParam("Type full name to find the pointer for.", Required = true)]
        string typeFullName)
    {
        if (ValidateRequired(scriptJsonPath, "scriptJsonPath", "Provide the full path to script.json.") is { } err1) return err1;
        if (ValidateRequired(typeFullName, "typeFullName", "Provide the type full name.") is { } err2) return err2;

        try
        {
            var text = await ctx.Analysis.GetClassPointerAsync(scriptJsonPath, typeFullName);
            return new ToolCallResult(text, new
            {
                scriptJsonPath,
                typeFullName,
