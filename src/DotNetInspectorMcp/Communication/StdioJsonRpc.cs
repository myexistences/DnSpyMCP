...
namespace DotNetInspectorMcp.Communication;

internal sealed class StdioJsonRpc : IDisposable
{
    ...
    private async Task<string> ReadLineFramedJsonAsync(byte firstByte, CancellationToken cancellationToken)
    {
        ...
        // Track brace/bracket depth so multi-line JSON is handled correctly
        ...
        while (true)
        {
            ...
            if (one[0] == (byte)'\n')
            {
                // Only break on newline if braces are balanced
                if (depth <= 0)
                    break;
                // If still inside nested braces, keep reading
                buffer.Add(one[0]);
                continue;
            }
            ...
            if (one[0] == (byte)'{' || one[0] == (byte)'[')
                depth++;
            else if (one[0] == (byte)'}' || one[0] == (byte)']')
            {
                depth--;
                if (depth <= 0)
                    break;
